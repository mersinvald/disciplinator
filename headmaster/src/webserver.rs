...
use actix_web::{
    dev::JsonConfig, http::Method, server, App, Error, HttpRequest, HttpResponse, Json, Path,
    ResponseError, State as RequestState,
};
use actix_web_async_await::{await, compat, compat2, compat3};

use actix_web::middleware::{self, Middleware, Started};
...
where
    M: Message<Result = Result<D, E>> + Send + 'static,
    D: Serialize + Send + 'static,
    E: Send + 'static,
    ServiceError: From<E>,
    DbExecutor: actix::Handler<M>,
{
    let db_result = await!(state.db.send(message))?;
...
async fn update_settings(
    json: Json<db::models::UpdateSettings>,
    req: HttpRequest<AppState>,
) -> HttpResult {
    let user_id = req.user_id()?;
    await!(db_response(
        req.state(),
        db::UpdateSettings::new(user_id, json)
    ))
}
...
async fn update_settings_fitbit(
    json: Json<db::models::UpdateFitbitCredentials>,
    req: HttpRequest<AppState>,
) -> HttpResult {
    let user_id = req.user_id()?;
    await!(db_response(
        req.state(),
        db::UpdateSettingsFitbit::from_json(user_id, json)
    ))
}
...
async fn update_user(json: Json<http::UpdateUser>, req: HttpRequest<AppState>) -> HttpResult {
    let user_id = req.user_id()?;
    let response = db_response_map(
        req.state(),
        db::UpdateUser::from_json(user_id, json),
        |mut user| {
            // Clean the passwd hash
            user.passwd_hash.clear();
            user
...
async fn override_activity(
    date: Path<NaiveDate>,
    overrides: Json<Vec<http::ActivityOverride>>,
    req: HttpRequest<AppState>,
) -> HttpResult {
...
pub fn start(
    ...
    let server = server::new(move || {
        let json_config = move |cfg: &mut (JsonConfig<AppState>, ())| {
            cfg.0.limit(4096).error_handler(|err, _req| {
                let err_msg = format!("{}", err);
                actix_web::error::InternalError::from_response(
                    err,
                    ServiceError::InvalidPayload { error: err_msg }.error_response(),
                )
                .into()
            });
        };
        ...
        .prefix("/1")
        .resource("/register", move |r| {
            r.method(Method::POST)
                .with_config(compat2(register), json_config)
        })
        .resource("/login", move |r| {
            r.method(Method::POST)
                .with_config(compat2(login), json_config)
        })
        ...
        .resource("/settings", move |r| {
            ...
            r.method(Method::POST)
                .with_config(compat2(update_settings), json_config);
        })
        .resource("/settings/fitbit", move |r| {
            r.middleware(AuthMiddleware);
            r.method(Method::POST)
                .with_config(compat2(update_settings_fitbit), json_config);
        ...
        .resource("/user", move |r| {
            ...
            r.method(Method::POST)
                .with_config(compat2(update_user), json_config);
        })
        .resource("/override/{date}", move |r| {
            r.middleware(AuthMiddleware);
            r.method(Method::POST).with(compat3(override_activity));
        })
        .resource("/user/validate_email/{email_token}", |r| {
            r.method(Method::GET).with(compat(validate_email))
        })
    })
...
impl<T> ExtractTokenHeader for HttpRequest<T> {
    fn token(&self) -> Result<Uuid, ServiceError> {
        ...
        let token = Uuid::from_str(token).map_err(|_| ServiceError::Unauthorized)?;
...
impl ExtractUserId for HttpRequest<AppState> {
    fn user_id(&self) -> Result<i64, ServiceError> {
        ...
        let token_map = self.state().token_map.borrow();
        let id = token_map
            .get(&token)
